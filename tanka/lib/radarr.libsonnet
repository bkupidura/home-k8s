{
    ...
    access_control+:: [
      {
        order: 0,
        rule: {
          domain: std.format('radarr.%s', std.extVar('secrets').domain),
          resources: ['^/api/.*$'],
  ...
  radarr: {
    ...
    cronjob_backup: $._custom.cronjob_backup.new('radarr', 'arr', '10 06 * * *', 'restic-secrets-default', 'restic-ssh-default', ['/bin/sh', '-ec', std.join(
      '\n',
